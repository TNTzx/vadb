...
export default () => {
    ...
    routerInfo.router.post("/", async (req: ExtendedReq, res: ExtendedRes) => {
        if (!await req.validate())
            return res.message(403, "You do not have permissions to add an artist." );

        if (req.headers["content-type"] === undefined)
            return res.message(400, "Missing content-type header parameter.")
        ...
        if (missingFields.length > 0) {
            return res.message(400, "A few fields were missing from this request.", missingFields)
        }

        if (Status[req.body.status] === undefined)
            return res.message(400, `Status is invalid. List of possible values: ${Object.values(Status).join(", ")}` );

        if (Availability[req.body.availability] === undefined)
            return res.message(400, `Availability is invalid. List of possible values: ${Object.values(Availability).join(", ")}`);

        let existingArtist = await Artist.FetchByName(req.body.name);

        if (existingArtist !== null)
            return res.message(409, `Artist with this name already exists. (${existingArtist.id} | ${existingArtist.safeName}: ${existingArtist.name})`);
    ...
    routerInfo.router.patch("/:id", async (req: ExtendedReq, res: ExtendedRes) => {
        if (!await req.validate())
            return res.message(403, "You do not have permission to edit an artist." );
        ...
        if (artist === null)
            return res.message(404, "This artist does not exist in the database.");
